...
### 추정
  - 6.1 **추정**
    - **`점추정과 구간추정`**
    > 모수를 추정하기위해 특정한 수치로 표현하는 점추정과 범위로 추정하는 구간추정이 있다.
      + 추정치 : 모수를 추정하기 위한 표본에서 도출된 통계량
      + 추정량 : 추정치를 계산하기 위한 함수
    - **`바람직한 점추정량의 조건`**
    > 점추정량은 그 값이 맞다면 정확하지만 아니면 틀린 추정이기에 오차를 필연적으로 동반하게된다.
      + 평균 오차제곱 : 평균에서 측정치를 뺀 값(오차)가 최소여야한다.
      + 불편성 : 추정치를 계산하기 위한 함수(추정량)와 모수가 같아야한다.
      + 일치성 : 표본의 크기가 모집단에 근접해야한다.
      + 유효성 : 추정량의 분산이 최소여야한다.
      + 충분성 : 표본은 모집단을 대표하는 성질을 지녀야한다.
    - **`구간추정`**
    > 점추정에서 발생하는 오차를 보완하기 위해 구간으로 모수를 추정하여 정확도를 높인다.
      + 신뢰수준 : 추정값이 존재하는 구간에 모수가 포함될 확률
      + 신뢰구간 : 신뢰수준을 기준으로 하한과 상한을 표시한다. 
      ```
      일반적으로 정규분포로 가정하니, z값은 표준정규분포에서 찾고 곡선아래의 면적 1은 100% 신뢰도를 의미한다.
      ```
  - 6.1 **모평균의 구간추정**
